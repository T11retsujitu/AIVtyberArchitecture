...
import { AgentResponseSchema } from '../response-schema.js';
...
export function createNimLlmClient(config: NimClientConfig): LlmClient {
  ...
  return {
    async complete(messages: ChatMessage[], schema): Promise<ReturnType<typeof AgentResponseSchema.parse>> {
      ...
      if (mode === 'json_schema') {
        body.response_format = {
          type: 'json_schema',
          json_schema: { name: 'agent_response', schema, strict: true },
        };
      } else {
      ...
      const attemptOnce = async (): Promise<ReturnType<typeof AgentResponseSchema.parse>> => {
        let res: Response;
        try {
        ...
        // 構造の最終強制（不変条件#3）。余剰キー/必須欠落はここで弾く。
        const result = AgentResponseSchema.safeParse(parsedJson);
        if (!result.success) {
          throw new NimError(`AgentResponse の形が不正: ${result.error.message.slice(0, 200)}`, true);
        }
        return result.data;
      };
