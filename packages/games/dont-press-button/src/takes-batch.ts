/**
 * dont-press-button / takes-batch.ts — Mode B+ 用に実 take を複数撮る（docs/13 §5・docs/12）
 *
...
async function main(): Promise<void> {
  ...
  // seed は 0/1 を循環（入りの揺らぎ）。各 seed を複数回撮って LLM のゆらぎを拾う。
  ...
  const summaries = await pool<{ id: string; seed: number }, TakeSummary>(jobs, concurrency, async (job) => {
    try {
      const trace: DreamTrace = await runAgentLoop(dontPressButton, deps, {
        seed: job.seed,
        provenance: {
          runId: job.id,
          createdAt,
      ...
      console.log(`  ✓ ${job.id} seed${job.seed}: ${trace.endReason} / ${trace.turns.length}ターン${trace.closing ? '＋締め' : ''}`);
      return s;
    } catch (err) {
